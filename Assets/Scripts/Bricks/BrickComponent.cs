...
public class BrickComponent : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.CompareTag("Ball") && !Globals.GamePaused)
        {
            ...
            ballVel = collision.gameObject.GetComponent<Rigidbody2D>().velocity; // get velocity of ball on impact
        }
    }
    ...
    private void OnTriggerStay2D(Collider2D collider)
    {
        if (collider.gameObject.name == "ExplodeRadius")
        {
            ...
            gameObject.AddComponent<Rigidbody2D>().AddRelativeForce(-ballVel, ForceMode2D.Impulse); //launch the affected bricks forward to simulate an explosion.
    ...
    IEnumerator Explode()
    {
        yield return new WaitForSeconds((Globals.RNG.Next(30, 40) / 100)); //wait for a random interval of time.
        state = 0;
    ...
    {
        int chance = Globals.RNG.Next(0, 100); //random change for powerup to appear.
        ...
        { // this halves the brick's scale every 0.1 in game seconds, to show a breaking effect, for 3 iterations.
            gameObject.transform.localScale = new Vector3(transform.localScale.x / 2, transform.localScale.y / 2, transform.localScale.z);
